...
def send_to_nr():
    ...
    p_parent = tracer.start_span(name=GLAB_SERVICE_NAME + " - pipeline: "+os.getenv('CI_PARENT_PIPELINE'), attributes=atts, start_time=do_time(str(pipeline_att['started_at'])), kind=trace.SpanKind.SERVER)
    try:
